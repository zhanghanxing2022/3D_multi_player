class Game{
	...
	constructor(socket,local_player,local_player_name,selected_model){
		this.local_player = local_player;
		this.local_player_name = local_player_name;
		this.socket = socket;
		if(selected_model!="Random")
		{
			this.local_selected = selected_model;
		}
		...
		//增加自己的人物模型
		this.Myanimate = [ 'Turn',  'Pointing', 'Talking', 'Pointing Gesture', 'Walking Backwards','Running','Walking','Idle'];
		const options = {
			assets:[
		...
		this.Myanimate.forEach( function(anim){ options.assets.push(`${game.assetsPath}fbx/animate/${anim}.fbx`)});
	...
	init() {
		...
		this.player = new PlayerLocal(this,this.local_selected);
	...
	loadEnvironment(loader){
		...
		loader.load(`${this.assetsPath}fbx/town.fbx`, function(object){
			...
			// rgbeLoader.load(`${this.assetsPath}evening_road_01_puresky_4k.hdr`,(texture) =>
			// {
			// 	console.log(texture)
			// 	console.log("aaaa")
	...
	//add
	loadNextAnimate(loader){
		let anim = this.Myanimate.pop();
		const game = this;
		loader.load( `${this.assetsPath}fbx/animate/${anim}.fbx`, function( object ){
			game.player.xGanimations[anim] = object.animations[0];
			if (game.anims.length>0){
				game.loadNextAnimate(loader);
			}else{
				delete game.Myanimate;
				// game.action = "Idle";
				// game.mode = game.modes.ACTIVE;
				game.animate();
			}
			...
			if (game.player.id != data.id){
				...
				if (iplayer===undefined){
					...
					if (rplayer===undefined){
						//Initialise player
						game.initialisingPlayers.push( new Player( game, data.name,data ));
					}else{
						//Player exists
...
class Player{
	constructor(game, the_name,options){
		this.local = true;
		let model, colour;
		...
		this.xGanimations = this.game.xGanimations;
		...
		loader.load( `${game.assetsPath}fbx/people/${model}.fbx`, function ( object ) {
			...
			if (player.local){
				...
				game.xGanimations.Idle = object.animations[0];
			...
			if (game.xGanimations.Idle!==undefined) player.action = "Idle";
		} );
		const text_loader = new THREE.FontLoader();
		text_loader.load(`${game.assetsPath}gentilis_bold.typeface.json`,(font)=>{
			console.log(font)
			const textGeo = new THREE.TextGeometry(
				the_name,  // 文字内容
				{
					...
					size: 10, // 文字大小
					height: 2, // 文字厚度
					curveSegments: 0.3,  // 文字平滑度
					bevelThickness: 0.2, // 文字斜面厚度
					bevelSize: 0.15,  // 文字斜面的大小
			...
			player.name= new THREE.Mesh(textGeo, materials);
			player.name.position.x = -the_name.length*3;
			player.name.position.y = 310;
			player.name.position.z = 0;
			player.name.rotation.x = 0;
			player.name.rotation.y = 0;
	...
	set action(name){
		...
		let clip;
		console.log(this.xGanimations)
		if(this.local)
		console.log(this.model);
		if(this.model=="xg")
		{
			clip = (this.local) ? this.xGanimations [name] : THREE.AnimationClip.parse(THREE.AnimationClip.toJSON(this.xGanimations[name])); 
		}else
		 clip = (this.local) ? this.animations[name] : THREE.AnimationClip.parse(THREE.AnimationClip.toJSON(this.animations[name])); 
	...
	update(dt){
		...
		if (this.game.remoteData.length>0){
			...
			for(let data of this.game.remoteData){
				...
				const euler2 = new THREE.Euler(data.pb, data.heading+1, data.pb);
...
class PlayerLocal extends Player{
	constructor(game, model){
		super(game, game.local_player_name,model);
