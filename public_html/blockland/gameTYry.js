class Game{
	...
	constructor(socket,local_player,local_player_name){
		...
		const game = this;
		//change
		this.anims = ['Walking', 'Walking Backwards', 'Turn', 'Running', 'Pointing', 'Talking', 'Pointing Gesture',"Idle"];
		...
		//change
		this.anims.forEach( function(anim){ options.assets.push(`${game.assetsPath}fbx/animate/${anim}.fbx`)});
	...
	loadNextAnim(loader){
		...
		loader.load( `${this.assetsPath}fbx/animate/${anim}.fbx`, function( object ){
			...
			object.position.set(3122, 150, -173);
			game.scene.add(object);
...
class Player{
	constructor(game, the_name,options){
		...
		// loader.load( `${game.assetsPath}fbx/people/${model}.fbx`, function ( object ) {
			loader.load( `${game.assetsPath}fbx/people/xg.fbx`, function ( object ) {
			...
			player.object.position.set(3122, 20, -173);
	...
	move(dt){
		...
		if (!blocked){
			if (this.motion.forward>0){
				const speed = (this.action=='Running') ? 300 : 150;
